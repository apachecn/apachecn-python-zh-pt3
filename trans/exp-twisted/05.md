...
这就是如何编写一个Twisted的 tap 插件。它将一个类标记为
...
Twisted的网早于所有这些。在 URL 模式匹配变得流行之前，将 web 资源视为一个`tree`也是一种选择——这就是 Twisted web 采用的选择。因此，它有一个包含子资源的资源模型。
