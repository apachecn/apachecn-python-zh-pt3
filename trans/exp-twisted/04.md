...
Docker工人和一般的集装箱都是新的。工具和关于如何使用工具的共识都在快速发展。我们在这里给出了如何使用 Docker 的基础，因此我们可以在此基础上建立如何使用 Twisted in Docker 的理解。
...
我们安装Twisted的。在 Twisted 的许多其他好处中，它包含一个方便的默认 web 服务器，这对演示很有用。
...
幸运的是，*任何*Twisted的程序都设置为 PID 1。这是因为 Twisted 的流程基础设施会自动收获预期的和意外的孩子。
...
NColony 正在监控的进程不一定是一个Twisted的进程，但在我们的例子中，它将是一个Twisted的进程——事实上，是另一个`tap`插件。
