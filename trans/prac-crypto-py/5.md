...
幸运的是，效率和完整性这后两个问题很容易解决。回想一下，我们不是为了*保密*而加密，而是为了*证明来源或真实性*。不加密消息本身，而是加密消息的一个*哈希*怎么样？
...
一旦我们有了哈希值 *h* ，我们就用 RSA 私钥对它进行加密。为了描述这个操作，我们将使用加密协议中常用的一些符号。具体来说，我们将使用`{` ⋅ `}`来表示 RSA 加密的数据。大括号内的所有内容都是明文，但是大括号表明明文在某个加密信封内。大括号还会有一个表示键的下标。所以比如密文 *C* 是在某个密钥 *K* 下加密的明文 *P* ，这个被描绘成*C*= {*P*}<sub>*K*</sub>。
