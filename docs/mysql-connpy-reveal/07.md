...
在某些情况下，应用可能会使用几个集合。一种解决方案是使用`get_collection()`逐个检索集合。不过，还有另一种方法。
...
当应用不再需要一个集合时，可以使用 schema 对象上的`drop_collection()`方法删除它。方法接受要删除的集合的名称。如果收藏不存在，`drop_collection()`会默默忽略。
