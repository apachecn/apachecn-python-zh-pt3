...
如果任务等待的时间确实比执行代码的时间长，那么 asyncio 或基于线程的并行化方法可能是最佳选择。根据经验，我建议对调用服务器但自己不等待网络请求的应用使用 asyncio，对接受入站连接的应用使用进程池和线程池的组合。 <sup>[12](#Fn12)</sup> 表示这一点的决策树如图 [7-10](#Fig10) 所示。
