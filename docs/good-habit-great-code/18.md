...
最快的排序是 nlog(n)阶。这实际上是 knlog(n)，其中 k 取决于编码效率、处理器速度等。这个表达式使得对数函数的基数看起来必须是 10。但基数是多少并不重要，因为只有一个 log 函数(你自己选)。所有其他的都是你选择的对数函数的倍数，例如![$$ {\log}_{10}(x)=c{\log}_2(x) $$](img/A461100_1_En_18_Chapter_IEq1.gif)，其中 c 不随 x 的变化而变化。你能计算出这个方程中 c 的数值吗？答案在脚注里。 [<sup>2</sup>](#Fn2) 
