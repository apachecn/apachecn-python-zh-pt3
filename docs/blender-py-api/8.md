...
在三维视口标题，我们可以导航到添加➤灯，以选择任何Blender的内置灯。使用 Python 工具提示，我们可以看到它们都依赖于函数`bpy.ops.object.lamp_add()`，用`type= parameter`决定光线的类型。我们有`SUN`、`POINT`、`SPOT`、`HEMI`和`AREA`等选项。每种类型都有自己的参数集需要配置。
