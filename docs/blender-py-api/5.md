...
场景变量将是`bpy.types.*`变量的实例。这些包括Blender类型`StringProperty`、`FloatProperty`、`IntProperty`和`BoolProperty`。任何时候面板通过调用`self.layout.prop`在 GUI 中包含一个变量，该变量将根据其类型进行逻辑格式化。整数和浮点数出现在滑动条中，字符串显示为文本输入字段，布尔值显示为复选框，等等。
