...
请注意，TLS 协议的`Finished`消息应该可以检测到这种攻击。发送包含握手期间交换的所有消息的哈希的消息的全部意义在于揭示这种操纵。
...
请注意，`tbs_certificate_bytes`是 DER 编码的(不是 PEM 编码的)字节，它们被哈希以签署证书。因此，在示例代码中，发行者的公钥用于检查证书中这些字节的签名。重复一遍，PEM 数据上的签名是*而不是*。
